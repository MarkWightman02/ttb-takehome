...
Verification localizes the Government Warning from the same OCR result first, then excludes lines inside or strongly overlapping that region from generic extraction. Candidate joins require panel consistency, vertical proximity, and horizontal alignment. ABV and metric volume use explicit patterns and normalize to percentage and milliliters. Class/type detection uses a small generic cue set rather than a purported regulatory taxonomy. Brand ranking combines compactness, line-height prominence, upper-panel position, OCR confidence, cue exclusions, and repetition in responsible-entity text. Similarly plausible brand geometry is retained for review rather than hidden by a confident selection.

Producer/bottler extraction anchors on a limited set of responsible-entity cues, accepts a same-line company, and examines spatially nearby lines in the same panel for an address. Address normalization handles presentation punctuation and deterministic U.S. state-name/abbreviation equivalence; partial containment is review rather than match. Country extraction anchors on `Product of`, `Produced in`, `Imported from`, and `Made in`. The request's explicit import flag controls applicability. Domestic origin results are `not_applicable` and are excluded from overall status aggregation. These utilities are deliberately not a role taxonomy, postal validator, geocoder, or complete origin-language catalog.
...
The generated evaluation corpus keeps rendering, expectations, and the real-Tesseract runner separate from production services. It covers spirits, wine, and malt-beverage layouts; domestic and imported applications; true mismatches; missing evidence; ambiguity; mild photographic degradation; and both left/right arrangements of product and warning panels. The runner reports false confident matches first, status differences per case and check, OCR failures, and stage latency. Generated images remain in memory and do not become repository artifacts. Exact generated-corpus status results are regression coverage, not a statistical estimate of real-label accuracy.
...
The absence of application persistence simplifies retention and keeps the prototype standalone. Authentication, COLAs Online integration, batch processing, and production governance are outside this slice. Tesseract currently uses English and page-segmentation mode 6 and may struggle with curved, reflective, rotated, stylized, or poorly photographed labels. The panel model is intentionally lightweight; overlapping panels, circular text, severe perspective, or weak column gaps can remain ambiguous. OCR confidence and local image heuristics are evidence rather than proof of typeface, physical dimensions, contrast under ordinary conditions, or final compliance. Deterministic extraction may produce review or not-found outcomes on unusual layouts. A deployed reviewer URL remains a final-product deliverable.
