...
| Repeatable quality evaluation | User | Implemented | Separate real-Tesseract runners for in-memory generated labels and checked-in real image/JSON pairs | Reports raw OCR, expected/extracted fields, statuses, false confident matches, OCR failures, bounded call counts, crop evidence, and stage latency; generated and six-example real metrics remain separate and are not presented as general real-world accuracy. |
